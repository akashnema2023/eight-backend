...
public class PlayListService {
    ...
    public ResponseEntity<?> createPlayList(long userId, String playListName, MultipartFile file) {
        if (userId > 0) {
            if (userRepository.existsById(userId)) {
                ...
                if (!file.isEmpty()) {
                    playListData.setPlayListImg(storageServices.uploadFile(path,file));
                }
                    ...
                    if (videoRepository.existsById(videoId)) {
                        if (userRepository.existsById(userId)) {
                            if (playListRepo.existsById(playListId)) {
                                ...
                                playListData.setPlayListImg(videoData.getThumbNailUrl());
                                playListRepo.save(playListData);
